...
- **`llm.called`** (интерпретатор и прочие вызовы LLM):
  ...
  - `prompt_version` зависит от задачи (напр. `cognitive-interpret-answer@v7`, `cognitive-card-render@v2`).
...
| **card.rendered** | LLM выдал полный текст карты §6 пользователю | session_id, max_user_id, card_type, card_text, prompt_version, llm_call_id, protocol_version, version, matched_types, model, provider | после card.computed; промпт `cognitive-card-render@v2` |
