...
- **`llm.called`** (интерпретатор и прочие вызовы LLM):
  - **`purpose: "answer_interpretation"`** — интерпретация протокольного ответа;
  - **`purpose: "card_render"`** — генерация полного текста карты §6 после `card.computed`;
  - для iter-3 при включённом флаге — генерация следующего вопроса (отдельный ключ идемпотентности);
  - интерпретатор: `idempotency_key = llm.called:${session_id}:${question_id}:interpret`;
  - рендер карты: `idempotency_key = llm.called:${session_id}:card.render`;
  - `prompt_version` зависит от задачи (напр. `cognitive-interpret-answer@v6`, `cognitive-card-render@v1`).
...
- **`card.computed`** (после 12/12):
  ...
  - payload: `card_type`, `confidence`, `confidence_resolution` (опц.), `confidence_message` (опц.), `input_answer_ids`, `version`, `protocol_version` (`"v1"`), `coordinates` (в т.ч. голоса целей, профиль модальности, якоря), `matched_types[]` (1–3 типа из таблицы §4.1; пусто, если `confidence < CARD_CONFIDENCE_THRESHOLD`), `synthetic_drawing`, `core_unformed`.

- **`card.rendered`** (после успешной записи `card.computed`, LLM):
  - `idempotency_key = card.rendered:${session_id}:${card_type}:${prompt_version}` (перерендер при смене версии промпта);
  - payload: `session_id`, `max_user_id`, `card_type`, `card_text` (markdown §6), `prompt_version`, `llm_call_id`, `protocol_version`, `version` (`card.rendered@v1`), `matched_types`, `model`, `provider`;
  - при ошибке LLM или `CARD_RENDER_ENABLED=false` пользователю уходит короткая строка (fallback), событие может отсутствовать.

---

### История правки 2026-04-30 (рендер §6)

Автоматический вывод полной карты по §6 через LLM (`card_render`, `card.rendered`); контракт десяти заголовков §6 зафиксирован в `prompts/cognitive-card-render.md`.
...
| **llm.called** | Вызов Claude/OpenAI | session_id, model, prompt_version, input_hash, latency_ms, purpose | до отправки сообщения (INV-06); `purpose ∈ {answer_interpretation, card_render, …}` |
...
| **card.rendered** | LLM выдал полный текст карты §6 пользователю | session_id, max_user_id, card_type, card_text, prompt_version, llm_call_id, protocol_version, version, matched_types, model, provider | после card.computed; промпт `cognitive-card-render@v1` |
