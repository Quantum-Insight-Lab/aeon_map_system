...
| **INV-03** | Имя типа в payload `card.computed` отображается только при `confidence ≥ CARD_CONFIDENCE_THRESHOLD`; иначе вместо типа — `confidence_message`. Confidence считается по пути алгоритма §4.2 (см. `src/aeon/cognitive-engine.ts::computeConfidence`). | aeon_engine, не LLM |
