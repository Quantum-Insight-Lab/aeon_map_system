...
export function formatProtocolQuestionMessageHtml(q: ProtocolQuestion): string {
  ...
  if (q.axis === 'goal' || q.axis === 'anchor') {
    ...
    const footer =
      q.axis === 'goal'
        ? `<i>Ответь <b>одной цифрой</b> — номер подходящего пункта от <b>1</b> до <b>8</b> (например: <b>6</b>).</i>`
        : `<i>Ответь <b>одной буквой</b> — одна из <b>А</b>–<b>З</b>, как у подходящей строки (например: <b>В</b>).</i>`;
...
export function formatProtocolQuestionMessageMarkdown(q: ProtocolQuestion): string {
  ...
  if (q.axis === 'goal' || q.axis === 'anchor') {
    ...
    const footer =
      q.axis === 'goal'
        ? `**Ответь одной цифрой** — номер подходящего пункта от **1** до **8** (например: **6**).`
        : `**Ответь одной буквой** — одна из **А**–**З**, как у подходящей строки (например: **В**).`;
    return `${stemMd}\n\n**Варианты:**\n\n${variantsMd}\n\n${footer}`;
  }
  if (q.axis === 'modality') {
